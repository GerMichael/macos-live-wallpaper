...
class VideoItem {
    static func getAVPlayerItem(for url: URL, videoCompositionConfig: VideoCompositor.Configuration?) async throws -> AVPlayerItem {
        ...
        if let videoCompositionConfig {
            let compositionResult = try await VideoCompositor.compose(for: aVURLAsset, options: videoCompositionConfig)
